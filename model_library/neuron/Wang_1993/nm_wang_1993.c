...
void nm_wang_1993_ina_f (double * vars, double * params, double * ret) {
    double aux = nm_wang_1993_mna(vars[WANG_V], params[WANG_SIGMA]);
    aux = aux * aux * aux;

    *ret = params[WANG_G_NA] * aux * vars[WANG_H_NA] * (vars[WANG_V] - params[WANG_V_NA]);
    return;
}
...
void nm_wang_1993_ik_f (double * vars, double * params, double * ret) {
    double aux = vars[WANG_N_K] * vars[WANG_N_K] * vars[WANG_N_K] * vars[WANG_N_K];
    *ret = params[WANG_G_K] * aux * (vars[WANG_V] - params[WANG_V_K]);
...
double nm_wang_1993_set_pts_burst (double pts_live, neuron_model * nm) {
    ...
    select_dt_neuron_model(dts, pts, length, pts_live, &(nm->params[WANG_DT]), &(nm->pts_burst));
    
    return nm->params[WANG_DT];
}
