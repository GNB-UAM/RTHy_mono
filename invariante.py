...
if args.plot_inv==True:
	...
	plt.scatter(periodoLP, PDtoLP_plot, label=label, c=np.linspace(0, len(periodoLP), len(periodoLP)), s=5, cmap=plt.get_cmap('Blues'))
	plt.colorbar()
	...
	plt.scatter(periodoLP, delay_plot, label=label, c=np.linspace(0, len(periodoLP), len(periodoLP)), s=5, cmap=plt.get_cmap('Reds'))
	plt.colorbar()
	...
	f_plot.write(args.n1 + ' ' + args.n2 + ' {:.5f}'.format(r_value**2*100) + ' {:.5f}'.format(r_value2**2*100) + ' {:.5f}'.format(slope) + ' {:.5f}'.format(slope2) + '\n')
	f_plot.close()
